...
function ranDat(start:number, end:number) : Date{
...
// mocking users
let user0 = new User(getId(), "Pavlosak12", "pavel@afdl.cfafd", EmailStatus.NOT_VERIFIED, "pavel", "pijkola", ranDat(getId(), getId()), ranDat(getId(), getId()), "If you have nothing, are you a nillionaire?", Subjects.ARTS, Country.AUSTRIA, "Vienna", gen(Generator.CASUAL), gen(Generator.RARE), Math.round(Math.random() * 100));
let user1 = new User(getId(), "Jacob__", "jacob@afdl.cfafd", EmailStatus.NOT_VERIFIED, "jakub", "sober", ranDat(getId(), getId()), ranDat(getId(), getId()), "Gas smells awful, You might as well live.", Subjects.CHEMISTRY, Country.CZECH_REPUBLIC, "Brno", gen(Generator.CASUAL), gen(Generator.RARE),Math.round(Math.random() * 100));
let user2 = new User(getId(), "Filipoz_", "filip@afdl.cfafd", EmailStatus.NOT_VERIFIED, "filip", "trůníček", ranDat(getId(), getId()), ranDat(getId(), getId()), "Let your day be neither manic nor volcanic.", Subjects.PHYSICS, Country.POLAND, "Varšava", gen(Generator.CASUAL), gen(Generator.RARE),Math.round(Math.random() * 100));
let user3 = new User(getId(), "KlaraNaKlarinetHrala43", "klarinet@afdl.cfafd", EmailStatus.NOT_VERIFIED, "klarinet", "skočdopolová", ranDat(getId(), getId()), ranDat(getId(), getId()), "Out of my mind. Back in five minutes.", Subjects.CZECH, Country.USA, "Ohio", gen(Generator.CASUAL), gen(Generator.RARE),Math.round(Math.random() * 100));
let user4 = new User(getId(), "12Barber", "barbra@afdl.cfafd", EmailStatus.NOT_VERIFIED, "barbora", "flašinetová", ranDat(getId(), getId()), ranDat(getId(), getId()), "A hangover is the wrath of grapes.", Subjects.MATHS, Country.HUNGARY, "Debrecen", gen(Generator.CASUAL), gen(Generator.RARE),Math.round(Math.random() * 100));
let user5 = new User(getId(), "JohnBlack", "johnysmrdijakdvazvony@afdl.cfafd", EmailStatus.NOT_VERIFIED, "jan", "cihlík", ranDat(getId(), getId()), ranDat(getId(), getId()), "Death and Taxes", Subjects.HISTORY, Country.LITHUANIA, "Kaunas", gen(Generator.CASUAL), gen(Generator.RARE),Math.round(Math.random() * 100));
let user6 = new User(getId(), "Fildiz7", "filip@afdl.cfafd", EmailStatus.VERIFIED, "františek", "štítek", ranDat(getId(), getId()), ranDat(getId(), getId()), "Two wrongs don’t make a right, but three lefts do.", Subjects.ENGLISH, Country.CZECH_REPUBLIC, "Pardubice", gen(Generator.CASUAL), gen(Generator.RARE),Math.round(Math.random() * 100));
let user7 = new User(getId(), "PepaZDepa007", "kohout43@afdl.cfafd", EmailStatus.VERIFIED, "josef", "kahount", ranDat(getId(), getId()), ranDat(getId(), getId()), "OK, so what’s the speed of dark?", Subjects.PE, Country.SLOVAKIA, "Poprad", gen(Generator.CASUAL), gen(Generator.RARE),Math.round(Math.random() * 100));
let user8 = new User(getId(), "AdamVesely8", "ejdam@afdl.cfafd", EmailStatus.VERIFIED, "adam", "kůltulič", ranDat(getId(), getId()), ranDat(getId(), getId()), "It’s a thinking universe.", Subjects.SPANISH, Country.CZECH_REPUBLIC, "Hradec Hrálové", gen(Generator.CASUAL), gen(Generator.RARE),Math.round(Math.random() * 100));
